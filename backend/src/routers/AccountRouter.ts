...
AccountRouter.route('/login').post(async (req, res, next) => {
  try {
    if (req.headers['content-type'] !== 'application/json')
      throw { status: 415, err: 'invalid_content_type' }
    ...
    let user = (await UserModel.findOne({
      [req.body.username.includes('@') ? 'email' : 'username']:
        req.body.username.toLowerCase(),
    }).exec()) as any
    if (!user) throw { status: 401, err: 'username_invalid' }

    let comparison = await argon2.verify(
    ...
    server.error(res, err.status || 500, err.err || err)
  }
})
...
AccountRouter.route('/register').post(async (req, res, next) => {
  try {
    ...
    if (!req.body.username.match(usernameRegex))
      throw { status: 400, err: 'username_invalid' }
    if (!req.body.email.match(emailRegex))
      throw { status: 400, err: 'email_invalid' }
    if (req.body.username.length <= 2)
      throw { status: 400, err: 'username_too_short' }
